...
object GraphLoader {
  ...
  def load(patients: RDD[PatientProperty], labResults: RDD[LabResult],
    ...
    val vertexs = sc.union(vertexPatient, diagnostic_vertices, medication_vertices, lab_vertices)

    // Now Create All Edges
    // Patient-Diagnosis Edges
    val edgePatientDiagnostic: RDD[Edge[EdgeProperty]] = diagnostics.map(f => ((f.patientID, f.icd9code), f.date, f.sequence)).keyBy(_._1).reduceByKey((f1, f2) => if (f1._2 > f2._2) f1 else f2).map(f => Edge(f._1._1.toLong, broadcast_diag_vertices_ids_mapped.value(f._1._2).toLong, PatientDiagnosticEdgeProperty(Diagnostic(f._1._1, f._2._2, f._1._2, f._2._3))))
    ...
    val edgePatientMedication: RDD[Edge[EdgeProperty]] = medications.map(f => ((f.patientID, f.medicine), f.date)).keyBy(_._1).reduceByKey((f1, f2) => (f1._1, math.max(f1._2, f2._2))).map(f => Edge(f._1._1.toLong, broadcast_medication_vertices_ids_mapped.value(f._1._2).toLong, PatientMedicationEdgeProperty(Medication(f._1._1, f._2._2, f._1._2))))
    ...
    val edgePatientLab: RDD[Edge[EdgeProperty]] = labResults.map(f => ((f.patientID, f.labName), f.date, f.value)).keyBy(_._1).reduceByKey((f1, f2) => if (f1._2 > f2._2) f1 else f2).map(f => Edge(f._1._1.toLong, broadcast_lab_vertices_ids_mapped.value(f._1._2).toLong, PatientLabEdgeProperty(LabResult(f._1._1, f._2._2, f._1._2, f._2._3))))
